...
func (n *repoNode) parsePaths(p *parser) error {
	for {
		...
		case t_eol:
			return nil
		default:
			return fmt.Errorf("line %d (1472910536): unexpected token type %v while parsing repo paths",t.l, t.t)
		}
	}
	return nil
}
	...
	for {
		...
		if t.t == t_string {
			...
			case "R", "RW", "RW+", "RW+D", "C", "RW+C", "RW+CD", "RWD", "RWC":
				...
				n.addRule(nn)
			default:
				return fmt.Errorf("line %d (1472910546): unexpected %v token, expected (%s)", t.l, t.t, t.s)
			}
		} else if t.t == t_dash {
			...
			return fmt.Errorf("line %d (1472910537): unexpected %v token, expected", t.l, t.t)
		}
	}
	return nil
}
